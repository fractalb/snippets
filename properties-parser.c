...
{
	return pctx->comment_prefix != NULL &&
	       strcmp(pctx->comment_prefix, "") != 0;
}
				...
				const struct parse_ctx *pctx)
{
	...
	if (have_comments(pctx) &&
	    starts_with(pctx->comment_prefix, line + _len)) {
		// printf("Skip Comment.\n");
		return NULL;
	}
			      ...
			      const struct parse_ctx *pctx)
{
	...
	while ((nread = getline(&line, &len, stream)) != -1) {
		if (len <= 2)
			continue;
		const char *_value =
			parse_key_value_from_line(line, len, key, pctx);
		if (_value == NULL)
			continue;
...
int main()
{
	...
	char value[MAX_VALUE_SIZE];
	if (parse_key_value_from_file("test.properties", key, value, &pctx) !=
	    0) {
